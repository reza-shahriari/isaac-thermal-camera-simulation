...
### Start here → `OC.3` · Optical focus and defocus

> **Two cubes, one in focus.** A `two_depth_slab` G-buffer fixture in `conftest.py` and `scripts/focus_demo.py`, which convolves it with `OC.2`'s bank and writes stills plus a focus-sweep **video**. No config and no pipeline yet — the point is a blurred image after three steps.

`OC.3` is phase X, size S, and unblocks 0 other step(s).

**Promoted over the mechanical order.** Carries the focus lane the owner asked for on 2026-09-23, having found that no camera in this repo has a focus distance at all. The mechanical key sorts the lane last -- it is phase X and each step unblocks only its own lane -- but every frame the project has ever rendered is in perfect focus at every range, which is a statement about the camera that is not true of any camera. Move this entry to the lane's next open step as it advances.

#### Then, in order — 78 open steps
...
| 4 | **`PT.10`** | PT | B | M | — | ready |
| 5 | **`SE.2`** | SE | B | M | — | ready |
| 6 | **`PH.9`** | PH | C | S | — | ready |
| 7 | **`TC.8`** | TC | C | S | — | ready |
| 8 | **`AT.6`** | AT | C | M | — | ready |
| 9 | **`AT.9`** | AT | C | M | — | ready |
| 10 | **`GT.7`** | GT | C | M | — | ready |
| 11 | **`PT.13`** | PT | C | M | — | ready |
| 12 | **`PT.16`** | PT | C | M | — | ready |
| 13 | **`XD.1`** | XD | X | S | 8 | ready |
| 14 | **`OC.4`** | OC | X | S | 6 | ready |
| 15 | **`EV.1`** | EV | X | M | 5 | ready |

…and 63 more — `python scripts/next_step.py --queue 40`.
...
| AT.12 | ✅ **done.** `irsim.atmosphere.cloud_deck`: a map of column depth at the LCL, each column given a top and the profile 6u(1−u), whose integral is exactly the thickness — so a vertical ray is ADR 0126 to the bit. The infrared band marches it; `cloud_volume` voxelises the same function to NanoVDB for the visible one. ADR 0127. | **Measured.** Cloud spans **19.7 K** (p1–p99) against the sheet's 1.25 K; emission level **12–834 m** above base, not 0; largest bin 55.4 % → **39.6 %**. Two corrections found by looking: the angular field makes fins; 48 steps alias. 18 cases. | AT.11 | L | A |
| AT.13 | **Probe whether a volume reaches the AOVs at all.** The infrared band is computed from AOVs and a volume is not a surface, so a ray through cloud may report no hit and fall through to ADR 0060's analytic background. Two leads: an RTX **Volumes** AOV, and Kit 107's fix for volumes contributing to **alpha**. | One cube with a cloud VDB and a flat target behind it, `PathTracing`, depth/instance/alpha/Volumes dumped side by side. Either the volume appears in a named plane, with dtype and checksum, or the refusal is recorded. In-engine. | AT.12, IG.3 | S | X |
