...
### Start here → `TC.1` · Thermal coupling

> **The tick survives conduction: a stability guard and an IMEX step.** `ThermalField` enforces the explicit bound of `two_node.py`; conduction — lateral, through-thickness, links — is stepped implicitly through one prefactored sparse solve per tick; radiation and convection keep the midpoint rule.

`TC.1` is phase P, size M, and unblocks 15 other step(s).

#### Then, in order — 110 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`TC.1`** | TC | P | M | 15 | ready |
| 2 | **`PT.18`** | PT | P | S | 10 | ready |
| 3 | **`TC.2`** | TC | P | M | 10 | `TC.1` |
| 4 | **`WM.1`** | WM | P | M | 10 | ready |
| 5 | **`WM.2`** | WM | P | M | 9 | `WM.1` |
| 6 | **`WM.3`** | WM | P | M | 8 | `WM.2` |
| 7 | **`TC.4`** | TC | P | S | 6 | `TC.2` |
| 8 | **`PH.4`** | PH | P | M | 6 | ready |
| 9 | **`TC.3`** | TC | P | M | 6 | `TC.2` |
| 10 | **`PT.7`** | PT | P | M | 5 | ready |
| 11 | **`TC.5`** | TC | P | M | 5 | `TC.3`, `TC.4` |
| 12 | **`PH.1`** | PH | P | M | 4 | ready |
| 13 | **`PH.5`** | PH | P | M | 4 | `PH.4` |
| 14 | **`TC.6`** | TC | P | M | 4 | `TC.5`, `PT.18` |
| 15 | **`PT.11`** | PT | P | L | 4 | `TC.1`, `PT.7` |

…and 95 more — `python scripts/next_step.py --queue 40`.
