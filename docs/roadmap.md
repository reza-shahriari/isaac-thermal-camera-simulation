...
### Start here → `AI.6` · Asset ingestion

> **Split a multi-material mesh into one prim per face group, so a render can carry it.** `AI.4` made the audit read subsets; the renderer still binds one material per prim, so a mesh with three of them renders as one and the audit's coverage describes an asset the frame cannot reproduce. `AI.5`'s con

`AI.6` is phase B, size M, and unblocks 0 other step(s).
...
| AI.6 | **Split a multi-material mesh into one prim per face group, so a render can carry it.** `AI.4` made the audit read subsets; the renderer still binds one material per prim, so a mesh with three of them renders as one and the audit's coverage describes an asset the frame cannot reproduce. `AI.5`'s connected-component regrouping is the machinery; the input is a face-index set rather than a component. | The committed subset fixture renders with three materials across one wall, measured off the instance-id plane. | AI.4, AI.5 | M | B |
