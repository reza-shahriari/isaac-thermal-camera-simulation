...
### Start here → `OC.10` · Optical focus and defocus

> **Thermal defocus.** Germanium dn/dT is about +400e-6 per K, some 250x a visible glass, so an unathermalised IR lens walks out of focus as its housing warms. Drive W020 from the housing temperature the chain already solves, with an `athermal` switch that cancels it.

`OC.10` is phase X, size M, and unblocks 0 other step(s).
