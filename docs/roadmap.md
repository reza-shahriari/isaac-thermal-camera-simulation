...
### Start here → `OC.9` · Optical focus and defocus

> **`autofocus` and `track` focus modes.** Passive contrast-maximising AF over the rendered frame with a hysteresis band, and a mode that holds focus on a named prim's range.

`OC.9` is phase X, size M, and unblocks 0 other step(s).
