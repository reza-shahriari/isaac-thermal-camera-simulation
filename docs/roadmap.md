...
### Start here → `PT.9` · Point-wise surface temperature

> 🟡 **half done (engine-free).** `quad_flight_pointwise.yaml` + its script regenerate the mission with the deck, belly and two arms as patched surfaces; schema v13 gives a surface a **speed schedule** so the flight drives its convection (ADR 0109). Curved binding is WM.3's, the frame IG.2's.
