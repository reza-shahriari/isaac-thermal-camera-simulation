...
| AT.15 | ✅ **done.** The deck is a field of *towers*: column depth rises with the field's excess over the condensation threshold and is clipped by the inversion, where it used to be a soft threshold — a flat-topped mesa. Band-limited at 400 m, tiled (no footprint edge), and the **dome marches it too**, so both bands read one cloud. ADR 0130. | **Measured.** Largest 0.25 K bin **53.9 % → 39.6 % → 6.4 %** of the frame; in-cloud spread **28.8 K**. Dome vs LWIR coverage was 26 % / 57 % of the same pixels, now identical by construction. 23 cases. | AT.12 | M | A |
