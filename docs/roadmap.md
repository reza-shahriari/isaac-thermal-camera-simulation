...
### Start here → `PT.23` · Point-wise surface temperature

> **The analytic point target delivers 0.7785 of its own model.** `IG.2`'s first in-sim run measured it: the chain returns that fraction of `excess_radiance` at **every** range, converged from frame 3.

`PT.23` is phase A, size M, and unblocks 0 other step(s).

#### Then, in order — 63 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.23`** | PT | A | M | — | ready |
| 2 | **`AI.2`** | AI | A | L | — | ready |
| 3 | **`AI.4`** | AI | B | S | — | ready |
...
| PT.23 | **The analytic point target delivers 0.7785 of its own model.** `IG.2`'s first in-sim run measured it: the chain returns that fraction of `excess_radiance` at **every** range, converged from frame 3. | Range- and position-independent to four figures across 400–3200 m, so it is one scalar, not geometry, the summation window or cos⁴ — the four targets are at different azimuths. Not 0.92 (τ_opt) nor 0.800 (the F/1.0 aperture ratio). Bisect the stages **engine-free** — `run_frame` needs only a G-buffer — then the in-sim test's `xfail(strict)` comes off. | — | M | A |
