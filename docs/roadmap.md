...
### Start here → `SC.19` · Sensor chain and published-reference anchoring

> **Defects that appear after calibration are not replaced (§10.4).** `replace_bad_pixels` runs on the exact defect mask, so no hot pixel ever reaches the output. Split factory defects (on the map) from late ones (`noise.bad_pixel_late_fraction`, off it) and give the ISP the map, not the truth.

`SC.19` is phase A, size S, and unblocks 1 other step(s).

#### Then, in order — 63 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`SC.19`** | SC | A | S | 1 | ready |
| 2 | **`SC.18`** | SC | A | M | 1 | ready |
| 3 | **`SC.20`** | SC | A | M | — | `SC.18`, `SC.19` |
