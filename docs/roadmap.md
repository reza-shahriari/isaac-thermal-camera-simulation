...
### Start here → `OC.4` · Optical focus and defocus

> **`FocusSpec` on `OpticsSpec`** — mode `infinity` (default), `hyperfocal` or `fixed` with its distance, plus `defocus_model` and `defocus_apply` on `FidelitySpec` so each is an ablation switch inside the config hash.

`OC.4` is phase X, size S, and unblocks 6 other step(s).
...
#### Then, in order — 77 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`OC.4`** | OC | X | S | 6 | ready |
| 2 | **`IG.2`** | IG | A | M | — | ready |
...
| 15 | **`OC.5`** | OC | X | M | 4 | `OC.4` |

…and 62 more — `python scripts/next_step.py --queue 40`.
