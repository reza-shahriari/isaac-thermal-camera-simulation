...
### Start here → `OC.8` · Optical focus and defocus

> **The clutter fallback and the error bound.** Where the hidden background is another object rather than sky or sea, fill by push-pull extrapolation from visible background neighbours, and **measure** what that costs in the occlusion band of width c/2.

`OC.8` is phase C, size M, and unblocks 0 other step(s).
