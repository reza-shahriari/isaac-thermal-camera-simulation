...
### Start here → `SC.17` · Sensor chain and published-reference anchoring

> **The housing is seen through the field, not added as one number (§8.2, revised 2026-09-26).** `self_emission_power` is one scalar and cos⁴ multiplies the scene alone, so a uniform scene colder than the housing shades with the wrong sign. Implement the §8.2 per-pixel form and load `optics.vignetting

`SC.17` is phase A, size M, and unblocks 2 other step(s).

#### Then, in order — 64 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`SC.17`** | SC | A | M | 2 | ready |
| 2 | **`SC.19`** | SC | A | S | 1 | ready |
| 3 | **`SC.18`** | SC | A | M | 1 | `SC.17` |
| 4 | **`SC.20`** | SC | A | M | — | `SC.17`, `SC.18`, `SC.19` |
...
| SC.17 | **The housing is seen through the field, not added as one number (§8.2, revised 2026-09-26).** `self_emission_power` is one scalar and cos⁴ multiplies the scene alone, so a uniform scene colder than the housing shades with the wrong sign. Implement the §8.2 per-pixel form and load `optics.vignetting_map` into RI. ADR. | Red: the raw corner/centre ratio is < 1 at every scene temperature. Green: > 1 for a scene colder than the housing, < 1 warmer, on-axis power unchanged to 1e-6, a measured RI map moves the ratio. | — | M | A |
...
| SC.19 | **Defects that appear after calibration are not replaced (§10.4).** `replace_bad_pixels` runs on the exact defect mask, so no hot pixel ever reaches the output. Split factory defects (on the map) from late ones (`noise.bad_pixel_late_fraction`, off it) and give the ISP the map, not the truth. | Red: zero stuck pixels after the ISP. Green: the late fraction reaches the 8-bit output as isolated full-scale pixels, the factory fraction leaves only the smoothed footprint, the sky-target bench counts them as clutter. | — | S | A |
