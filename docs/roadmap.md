...
### Start here → `WM.2` · Warp mesh surface parameterisation

> **`TriangleMeshField` in `irsim.thermal`**, engine-free, composing `ThermalField` as `PlanarThermalField` does so the fixed tick and never-mutate-on-query rule carry over. Cells per face with a per-face resolution, Ptex style. `PlanarPatch` stays as the special case.

`WM.2` is phase P, size M, and unblocks 9 other step(s).

#### Then, in order — 87 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`WM.2`** | WM | P | M | 9 | ready |
| 2 | **`WM.3`** | WM | P | M | 8 | `WM.2` |
| 3 | **`PH.5`** | PH | P | M | 4 | ready |
| 4 | **`WM.5`** | WM | P | S | 1 | `WM.3` |
| 5 | **`PH.6`** | PH | P | M | 1 | `PH.5` |
| 6 | **`PH.7`** | PH | P | M | 1 | `PH.5` |
| 7 | **`PT.14`** | PT | P | S | — | `WM.5` |
| 8 | **`PH.8`** | PH | P | M | — | `PH.7` |
| 9 | **`WM.4`** | WM | P | M | — | `WM.3` |
| 10 | **`WM.6`** | WM | P | M | — | `WM.3` |
| 11 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 12 | **`AT.10`** | AT | A | M | — | ready |
| 13 | **`GT.2`** | GT | A | M | — | ready |
