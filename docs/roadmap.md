...
### Start here → `SC.18` · Sensor chain and published-reference anchoring

> **The FFC snapshots the shutter and the housing; the residual is radial (§11.2, revised 2026-09-26).** `FfcController` records only T_FPA and `NucResidual` is white. Carry T_shutter as a state, snapshot the §11.2 offset at each event, make the residual its three-term form; the white ΔT_FPA term stay
