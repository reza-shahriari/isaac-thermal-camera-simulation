...
### Start here → `AI.2` · Asset ingestion

> **Put the Phantom 4 in a scene.** 🟡 **Mostly shipped:** schema v16 binds a field to a prepared asset's prims (ADR 0132) and `render_phantom4.py` flies it in LWIR and colour, mounted by the rotation its own `world_frame:` implies (ADR 0133). **Left:** 234,923 solved cells reach no pixel — `MeshPointB
