...
### Start here → `PT.17` · Point-wise surface temperature

> **Patches solved from the scene config, not from Python.** `_build_thermal_field` reads `SurfaceSpec.patch` and builds a `PlanarThermalField` per patched surface on its library material; bindings reach `IrCamera` from `Scene`; the car scenes declare bonnet and road in YAML.

`PT.17` is phase P, size M, and unblocks 19 other step(s).

#### Then, in order — 112 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.17`** | PT | P | M | 19 | ready |
| 2 | **`PT.8`** | PT | P | S | 16 | ready |
| 3 | **`TC.1`** | TC | P | M | 15 | `PT.8` |
| 4 | **`PT.18`** | PT | P | S | 10 | `PT.17` |
| 5 | **`TC.2`** | TC | P | M | 10 | `TC.1` |
| 6 | **`WM.1`** | WM | P | M | 10 | ready |
| 7 | **`WM.2`** | WM | P | M | 9 | `WM.1` |
| 8 | **`WM.3`** | WM | P | M | 8 | `WM.2` |
| 9 | **`TC.4`** | TC | P | S | 6 | `TC.2` |
| 10 | **`PH.4`** | PH | P | M | 6 | ready |
| 11 | **`TC.3`** | TC | P | M | 6 | `TC.2`, `PT.17` |
| 12 | **`PT.7`** | PT | P | M | 5 | ready |
| 13 | **`TC.5`** | TC | P | M | 5 | `TC.3`, `TC.4` |
| 14 | **`PH.1`** | PH | P | M | 4 | `PT.17` |
| 15 | **`PH.5`** | PH | P | M | 4 | `PH.4` |

…and 97 more — `python scripts/next_step.py --queue 40`.
