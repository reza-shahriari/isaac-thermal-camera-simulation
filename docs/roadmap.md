...
### Start here → `PT.8` · Point-wise surface temperature

> **Bound the tick history.** `ThermalField._ticks` is appended to and never pruned; only the two ticks bracketing a query are read. A two-tick ring plus an optional history hook keeps `state_hash` working.

`PT.8` is phase P, size S, and unblocks 16 other step(s).

#### Then, in order — 111 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.8`** | PT | P | S | 16 | ready |
| 2 | **`TC.1`** | TC | P | M | 15 | `PT.8` |
| 3 | **`PT.18`** | PT | P | S | 10 | ready |
| 4 | **`TC.2`** | TC | P | M | 10 | `TC.1` |
| 5 | **`WM.1`** | WM | P | M | 10 | ready |
| 6 | **`WM.2`** | WM | P | M | 9 | `WM.1` |
| 7 | **`WM.3`** | WM | P | M | 8 | `WM.2` |
| 8 | **`TC.4`** | TC | P | S | 6 | `TC.2` |
| 9 | **`PH.4`** | PH | P | M | 6 | ready |
| 10 | **`TC.3`** | TC | P | M | 6 | `TC.2` |
| 11 | **`PT.7`** | PT | P | M | 5 | ready |
| 12 | **`TC.5`** | TC | P | M | 5 | `TC.3`, `TC.4` |
| 13 | **`PH.1`** | PH | P | M | 4 | ready |
| 14 | **`PH.5`** | PH | P | M | 4 | `PH.4` |
| 15 | **`TC.6`** | TC | P | M | 4 | `TC.5`, `PT.18` |

…and 96 more — `python scripts/next_step.py --queue 40`.
