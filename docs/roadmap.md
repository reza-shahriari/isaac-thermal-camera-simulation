...
> **In-sim test for the point bridge and the car demo.** 🟡 **Mostly shipped:** the tautology is gone — the oracles are the camera's three named world axes and an authored wall, and `position_frame_residuals` decodes with `world_positions` rather than a copy. **Left:** `test_position_frame_isaac.py` is
...
| IG.2 | **In-sim test for the point bridge and the car demo.** 🟡 **Mostly shipped:** the tautology is gone — the oracles are the camera's three named world axes and an authored wall, and `position_frame_residuals` decodes with `world_positions` rather than a copy. **Left:** `test_position_frame_isaac.py` is written and has never been run. | **Measured, engine-free.** Flipping the decode turns **7** red; flipping it *and* `pinhole_rays` together — what the old tests missed — still turns **5**. A wall decodes onto its authored plane to 1e-9 m. In-sim not run. | IG.1 | M | A |
