...
### Start here → `AT.12` · Atmosphere, sky and materials

> **A cloud with a third dimension, the same one in both bands.** A plane-parallel deck has no sides, so AT.11's opaque core is flat to a kelvin — wrong at an oblique aim, where a cumulus field is towers. Build a deck from the same `SkyFixedCloud`, march it for the infrared and voxelise it for the vis

`AT.12` is phase A, size L, and unblocks 2 other step(s).

#### Then, in order — 71 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`AT.12`** | AT | A | L | 2 | ready |
| 2 | **`IG.2`** | IG | A | M | — | ready |
...
| AT.12 | **A cloud with a third dimension, the same one in both bands.** A plane-parallel deck has no sides, so AT.11's opaque core is flat to a kelvin — wrong at an oblique aim, where a cumulus field is towers. Build a deck from the same `SkyFixedCloud`, march it for the infrared and voxelise it for the visible. | Red today: the visible cloud is painted into the dome at infinity, so no ray has a length inside it. After: a `UsdVol.Volume` a camera can fly toward, RGB by `PathTracing`, an LWIR frame spanning ≥ 5 K in cloud, a vertical ray still AT.11 to the bit. | AT.11 | L | A |
