...
### Start here → `OC.7` · Optical focus and defocus

> **The analytic hidden layer.** Sky and sea radiance are functions of ray direction this pipeline already evaluates, so the background behind a defocused silhouette can be **computed exactly** rather than inpainted — `SkyModel.clear_radiance` takes the elevation the G-buffer carries. No second render

`OC.7` is phase B, size M, and unblocks 0 other step(s).
