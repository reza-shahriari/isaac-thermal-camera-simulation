...
### Start here → `WM.3` · Warp mesh surface parameterisation

> **`MeshPointBridge`**: per-pixel instance id selects the prim's `wp.Mesh`, the query gives `(face, u, v)`, the field gives the temperature. Additive like `point_bridge`: an unbound prim keeps the per-instance path bit-identically.

`WM.3` is phase P, size M, and unblocks 8 other step(s).

#### Then, in order — 86 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`WM.3`** | WM | P | M | 8 | ready |
| 2 | **`PH.5`** | PH | P | M | 4 | ready |
| 3 | **`WM.5`** | WM | P | S | 1 | `WM.3` |
| 4 | **`PH.6`** | PH | P | M | 1 | `PH.5` |
| 5 | **`PH.7`** | PH | P | M | 1 | `PH.5` |
| 6 | **`PT.14`** | PT | P | S | — | `WM.5` |
| 7 | **`PH.8`** | PH | P | M | — | `PH.7` |
| 8 | **`WM.4`** | WM | P | M | — | `WM.3` |
| 9 | **`WM.6`** | WM | P | M | — | `WM.3` |
| 10 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 11 | **`AT.10`** | AT | A | M | — | ready |
| 12 | **`GT.2`** | GT | A | M | — | ready |
