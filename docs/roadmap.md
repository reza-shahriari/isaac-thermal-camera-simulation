...
### Start here → `AT.18` · Atmosphere, sky and materials

> **The mapping stops defaulting to a mirror.** `*metal*` and `*alumin*` resolve to a matte/painted entry rather than `bare_aluminium`, and `audit_materials.py` raises when a prim reaches an ε < 0.2 material through a *pattern* instead of an explicit per-asset override.

`AT.18` is phase A, size S, and unblocks 0 other step(s).

#### Then, in order — 68 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`AT.18`** | AT | A | S | — | ready |
| 2 | **`AI.2`** | AI | A | L | — | ready |
| 3 | **`AI.4`** | AI | B | S | — | ready |
| 4 | **`IG.16`** | IG | B | M | — | ready |
| 5 | **`TC.8`** | TC | C | S | — | ready |
| 6 | **`AT.17`** | AT | C | M | — | ready |
| 7 | **`AT.6`** | AT | C | M | — | ready |
| 8 | **`AT.9`** | AT | C | M | — | ready |
...
| **A — Aerial to the bar** | `AI.1`, `AI.2`, `AI.5`, `PT.1`, `PT.2`, `PT.5`, `PT.9`, `PT.23`, `AT.1`–`AT.5`, `AT.10`–`AT.12`, `AT.15`, `AT.16`, `AT.18`, `SC.1`–`SC.4`, `IG.2`, `IG.6`, `IG.13`, `GT.1`, `GT.2` | **CPU only.** An aerial scene config plus one command produces float32 frames whose target carries a gradient across one prim, with a per-pixel slant path behind it |
...
| AT.18 | **The mapping stops defaulting to a mirror.** `*metal*` and `*alumin*` resolve to a matte/painted entry rather than `bare_aluminium`, and `audit_materials.py` raises when a prim reaches an ε < 0.2 material through a *pattern* instead of an explicit per-asset override. | **Measured.** A 300 K housing under a 250 K sky reads **256.0 K at ε 0.09 against 294.1 K at ε 0.85 — 38.0 K, some 760 × NETD**. `phantom4.yaml` and `phantom3.py:249` already hand-correct this trap per asset; after, a new asset cannot inherit it silently. | — | S | A |
---
...
| SC.16 | **The camera is in the weather too (§9.5).** A lumped camera node driven by the *same* `WeatherSeries` as the scene — dissipation, solar load and h_c(v)·A forced convection — feeding T_FPA and so ADR 0016's existing drift path. Switchable off; an empirical fit, not an airframe model. | Red today: nothing in `detector/`, `isp/` or `noise/` reads wind, so a camera at 8.5 m s⁻¹ drifts exactly as much as one in still air. After: bias moves with wind in the sign and order of [R44]'s −1.02 → +3.86 °C, and a step in wind relaxes with the fitted time constant. | — | M | X |
