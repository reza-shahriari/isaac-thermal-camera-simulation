...
### Start here → `OC.2` · Optical focus and defocus

> **Hopkins OTF and the band-averaged kernel bank.** The defocus OTF as its quadrature, averaged over the band response the radiometry already loads, on a W020 grid and interpolated — with `geometric` (jinc) and `gaussian` selectable beside it.

`OC.2` is phase X, size M, and unblocks 8 other step(s).
