...
- **The position decode's in-sim half is written but unrun (`IG.2`).**
  `tests/integration/test_position_frame_isaac.py` renders the pitched, off-origin geometry scene
  and checks the float32 position plane against the stage -- a plate must decode onto the plane it
  was *authored* on, and a patch bound to it must gather a ramp across one prim -- which is the one
  oracle that owes nothing to either the renderer or the decode. It has never been executed: this
  workstation is shared and Isaac Sim was not started. The engine-free half is measured, so the
  arithmetic is pinned; what is unconfirmed is that *this build* still delivers the frame the
  pipeline assumes. `make test-all` on a free machine closes it.
